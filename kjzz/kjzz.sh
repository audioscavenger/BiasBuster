...
scheduler() {
  ...
  [ -n "${jsonSchedule}" ] && jsonSchedule=${ROOT}/${curyear}/KJZZ-schedule.json
...
kjzzDownloader() {
  ...
  [ $KJZZ_PID_RUNNING == true -a $KJZZ_FILE_LOCK == true ]  && echo "$curyear-$curmon-$curday $curHHMM kjzzDownloader $G$PID$END is appending \"${filename}\" for the next $HIGH$((secUntilNextHalf / 60))$END mm = $secUntilNextHalf s. stop it?" 1>&2 && RETURN=1
  [ $KJZZ_PID_RUNNING == true -a $KJZZ_FILE_LOCK == false ] && {
    [ "${filename%%Blues*}" != "${filename}" -o "${filename%%Jazz*}" != "${filename}" ] && {
      ...
      echo "$curyear-$curmon-$curday $curHHMM ERROR: kjzzDownloader $G$PID$END is running but NOT appending \"${filename}\". stop it?" | tee -a $LOG 1>&2
      RETURN=2
    }
  }
  ...
  [ $KJZZ_PID == false -a $KJZZ_FILE_LOCK == true ] && {
    ...
    echo "$curyear-$curmon-$curday $curHHMM kjzz.pid $G$PID$END was missing, is appending \"${filename}\" for the next $HIGH$((secUntilNextHalf / 60))$END mm = $secUntilNextHalf s. stop it?" | tee -a $LOG 1>&2
    echo $PID >${PIDFILE}
    RETURN=5
  }
...
export jsonSchedule=${ROOT}/$(date +%Y)/KJZZ-schedule.json
